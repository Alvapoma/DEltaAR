    ...
    while True:
        ...
        if  ispath == False and data[0].isdigit():
            ...
            if data[0] == '1' :
                ...
                reply = deltaMovemen.robotdelta(int(dataMessage[1]) ,int(dataMessage[2]) ,int(dataMessage[3]),dataMessage[4] ,dataMessage[5])+"\n"
            else:
                # data[0] == '1' envia angulos
                print("moviendo Angulos")
                auxarray = deltaMovemen.calcForward(int(dataMessage[1]) ,int(dataMessage[2]) ,int(dataMessage[3]))
                if auxarray[0]== 0:
                    lastValue[0] = int(auxarray[1])
                    lastValue[1] = int(auxarray[2])
                    lastValue[2] = int(auxarray[3])
                    deltaMovemen.movedelta(dataMessage[1] ,dataMessage[2] ,dataMessage[3],dataMessage[4] ,dataMessage[5])
            ...
            if len(dataMessage)>6:
                ...
                while valuedat< len(dataMessage) :
                    if dataMessage[0+valuedat] == '1' :
                        #cuando es Lineal
                        reply = deltaMovemen.robotdeltaline(int(dataMessage[1+valuedat]) ,int(dataMessage[2+valuedat]) ,int(dataMessage[3+valuedat]),dataMessage[4+valuedat] ,dataMessage[5+valuedat])+"\n"
                    
                    else:
                        # data[0] == '0' cuando es ptp
                        reply = deltaMovemen.robotdelta(int(dataMessage[1+valuedat]) ,int(dataMessage[2+valuedat]) ,int(dataMessage[3+valuedat]),dataMessage[4+valuedat] ,dataMessage[5+valuedat])+"\n"
                    valuedat=valuedat+6
            else:
                if data[0] == '1' :
                    #cuando es Lineal
                    reply = deltaMovemen.robotdeltaline(int(dataMessage[1]) ,int(dataMessage[2]) ,int(dataMessage[3]),dataMessage[4] ,dataMessage[5])+"\n"
                else:
                    # data[0] == '0' cuando es ptp
                    reply = deltaMovemen.robotdelta(int(dataMessage[1]) ,int(dataMessage[2]) ,int(dataMessage[3]),dataMessage[4] ,dataMessage[5])+"\n"
            ...
            angulo = deltaMovemen.cinematica_inv(lastValue[0],lastValue[1],lastValue[2])
